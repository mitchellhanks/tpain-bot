...
from TPBOT import dispatcher, OWNER_ID, LOGGER, spamcheck, IS_DEBUG
try:
	from TPBOT import SPAMWATCH_TOKEN
except:
	pass
from TPBOT.modules.helper_funcs.chat_status import user_admin, is_user_ban_protected, bot_can_restrict
from TPBOT.modules.helper_funcs.misc import build_keyboard_parser, revert_buttons
from TPBOT.modules.helper_funcs.msg_types import get_welcome_type
...
from TPBOT.modules.helper_funcs.welcome_timeout import welcome_timeout
from TPBOT.modules.log_channel import loggable

import TPBOT.modules.sql.feds_sql as fedsql
from TPBOT.modules.helper_funcs.alternate import send_message, leave_chat
...
*Admin only:*
 ...
 - /welcomemute <on/off>: all users that join, get muted; a button gets added to the welcome message for them to unmute themselves. This proves they aren't a bot!
 ...
 - /resetmutetext: - Reset unmute button text to default.
...
def send(update, message, keyboard, backup_message):
	...
	try:
		...
		msg = dispatcher.bot.send_message(chat.id, markdown_parser(backup_message +
																"\nNOTE: The current message was invalid "
																"due to markdown issues.  Could be due to the user's name. "),
												  reply_to_message_id=reply, 
												  parse_mode=ParseMode.MARKDOWN)
	except KeyError:
		msg = dispatcher.bot.send_message(chat.id, markdown_parser(backup_message +
																"\nNOTE: The current message is invalid "
																"due to an issue with some misplaced curly brackets. Please update "),
												  reply_to_message_id=reply, 
												  parse_mode=ParseMode.MARKDOWN)
	except BadRequest as excp:
		if excp.message == "Button_url_invalid":
			msg = dispatcher.bot.send_message(chat.id, markdown_parser(backup_message +
																"\nNOTE: The current message had an invalid url "
			...
			try:
				...
				if IS_DEBUG:
					print("Cannot send welcome msg in {} ({})".format(chat.title, chat.id))
			...
			# Don't welcome yourself
				...
				if welc_type != sql.Types.TEXT and welc_type != sql.Types.BUTTON_TEXT:
					...
					try:
						...
						sent = send_message(update.effective_message, "Note: An error occured when sending the custom message. Please update.")
					return
				else:
					# else, move on
					first_name = new_mem.first_name or "PersonWithNoName"  # edge case of empty name - occurs for some bugs.
...
def check_bot_button(update, context):
	...
	if getalluser and user.id in list(getalluser) and getalluser[user.id] == False:
		...
		if getalluser.get(user.id) and getalluser.get(user.id) == True:
			...
			try:
				query.answer(text="You are not a new member!")
			except BadRequest as err:
				print("-> Failed: {}".format(err))
				return
	"""
		...
		if left_mem:
			...
			if goodbye_type != sql.Types.TEXT and goodbye_type != sql.Types.BUTTON_TEXT:
				...
				try:
					...
					send_message(update.effective_message, "Note: An error occured when sending the custom message. Please update.")
				return
...
def security(update, context):
	...
	if len(args) >= 1:
		...
		if (var == "yes" or var == "ya" or var == "on"):
			...
			send_message(update.effective_message, "Please write `on`/`off`!", parse_mode=ParseMode.MARKDOWN)
	else:
		getcur, extra_verify, cur_value, timeout, timeout_mode, cust_text = sql.welcome_security(chat.id)
		if cur_value[:1] == "0":
			cur_value = "Forever"
		text = "Current settings:\nWelcome security: `{}`\nVerify security: `{}`\nMember will be muted during: `{}`\nTime verification timeout: `{}` ({})\nCustom unmute key: `{}`".format(getcur, extra_verify, cur_value, make_time(int(timeout)), "kick" if 1 else "banned", cust_text)
		send_message(update.effective_message, text, parse_mode="markdown")
...
def security_mute(update, context):
	...
	if len(args) >= 1:
		...
		if str(cur_value) == "0":
			send_message(update.effective_message, "Current settings: new member will be muted forever until they press the welcome button!")
		else:
			send_message(update.effective_message, "Current settings: new member will be muted for {} or until they press the welcome button!".format(cur_value))
...
def welcome(update, context):
	...
	if len(args) == 0 or args[0].lower() == "noformat":
		...
		if getcur:
			...
			welcsec = "Disabled "
		if cur_value[:1] == "0":
			welcsec += "(muted forever until pressing the unmute button)"
		else:
			welcsec += "(muted for {})".format(cur_value)
		text = "This chat's welcome setting: `{}`\n".format(pref)
		text += "Deleted the old welcome message: `{}`\n".format(prev_welc)
		text += "Currently deleting service messages: `{}`\n".format(cleanserv)
		text += "Currently muting new users when they join: `{}`\n".format(welcsec)
		text += "New users must verify the captcha/emoji: `{}`\n".format("Enabled " if extra_verify else "Disabled ")
		text += "The welcome mute button will say: `{}`\n".format(cust_text)
		...
		if args[0].lower() in ("on", "yes"):
			...
			send_message(update.effective_message, "I only understand 'on/yes' or 'off/no'!")
...
def reset_goodbye(update, context) -> str:
	...
	send_message(update.effective_message, "Successfully set custom goodbye message!")
	return "<b>{}:</b>" \
		   "\n#RESET_GOODBYE" \
		   "\n<b>Admin:</b> {}" \
...
def welcome_help(update, context):
	send_message(update.effective_message, WELC_HELP_TEXT.format(dispatcher.bot.username), parse_mode=ParseMode.MARKDOWN)
...
# 					return
# 				text = "*New FedBan*\n*Fed:* `TeamNusantaraDevs`\n*FedAdmin*: {}\n*User:* {}\n*User ID:* `{}`\n*Reason:* [CAS ban](https://combot.org/cas/query?u={})".format(mention_markdown(692882995, "TPBOT"), mention_markdown(user_id, user.first_name + (" " + user.last_name if user.last_name != None else "")), user_id, user_id)
...
# 			LOGGER.warning("Cannot fban spammer user!")
# 			return
# 		text = "*New FedBan*\n*Fed:* `TeamNusantaraDevs`\n*FedAdmin*: {}\n*User:* {}\n*User ID:* `{}`\n*Reason:* [SpamWatch] {}".format(mention_markdown(692882995, "TPBOT"), mention_markdown(user_id, user.first_name + (" " + user.last_name if user.last_name != None else "")), user_id, json.get('reason') if json.get('reason') else "Unknown reason")
# 		context.bot.sendMessage(-1001338861977, text, parse_mode="markdown", disable_web_page_preview=True)
...
dispatcher.add_handler(CommandHandler("welcome", welcome, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("goodbye", goodbye, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("setwelcome", set_welcome, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("setgoodbye", set_goodbye, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("resetwelcome", reset_welcome, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("resetgoodbye", reset_goodbye, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("cleanwelcome", clean_welcome, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("welcomehelp", welcome_help))
dispatcher.add_handler(CommandHandler("welcomemute", security, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("welcomemutetime", security_mute, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("setmutetext", security_text, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("resetmutetext", security_text_reset, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("cleanservice", cleanservice, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CallbackQueryHandler(check_bot_button, pattern=r"check_bot_"))
#dispatcher.add_handler(CallbackQueryHandler(WELC_EDITBTN, pattern=r"set_welc"))
