...
from TPBOT import dispatcher, updater, spamcheck, LOGGER
from TPBOT.modules.disable import DisableAbleCommandHandler
from TPBOT.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin
...
from TPBOT.modules.sql import admin_sql as sql
...
*Admin only:*
 - /pin: silently pins the message replied to - add 'loud' or 'notify' to give notification to users.
...
def demote(update, context):
	...
	if not user_member.status == 'administrator':
		send_message(update.effective_message, "I can't demote someone lower than non-admin!")
		return ""
...
def adminlist(update, context):
	...
	text = "Admins of *{}*:".format(update.effective_chat.title or "this chat")
...
def permanent_pin_set(update, context):
	...
	if conn:
		...
		if not args:
			...
			send_message(update.effective_message, text_maker)
			return ""
		prev_message = args[0]
		if prev_message == "off":
...
dispatcher.add_handler(CommandHandler("pin", pin, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("unpin", unpin, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("permapin", permapin, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("invitelink", invite, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("promote", promote, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("demote", demote, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(CommandHandler("permanentpin", permanent_pin_set, pass_args=True, filters=Filters.chat_type.groups))
dispatcher.add_handler(MessageHandler(Filters.status_update.pinned_message | Filters.user(777000), permanent_pin))
dispatcher.add_handler(DisableAbleCommandHandler(["adminlist", "admins"], adminlist))
