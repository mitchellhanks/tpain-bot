...
import TPBOT.modules.sql.connection_sql as sql
from TPBOT import dispatcher, LOGGER, SUDO_USERS, spamcheck
...
__help__ = """
Issue commands to a chat remotely (e.g. from the bot's PM or from a separate group).

 - /connect <chatid/tag>: Connect to remote chat
 - /connection: Show existing connection status
...
def connect_chat(update, context):
    ...
    if update.effective_chat.type == 'private':
        if args and len(args) >= 1:
            ...
            if gethistory:
                buttons = [InlineKeyboardButton(text="❎ Close button", callback_data="connect_close"), InlineKeyboardButton(text="🧹 Clear history", callback_data="connect_clear")]
            else:
                buttons = []
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                ...
                try:
                    ...
                    context.bot.send_message(update.effective_message.from_user.id, "You have connected with *{}*. Use /connection to see current available commands.".format(chat_name), parse_mode="markdown")
                except BadRequest:
                    pass
...
def help_connect_chat(update, context):
    ...
    if update.effective_message.chat.type != "private":
        ...
        send_message(update.effective_message, supportedcmd, parse_mode="markdown")
    ...
    if connect_match:
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                conn_chat = dispatcher.bot.getChat(connected(context.bot, update, chat, user.id, need_admin=False))
                chat_name = conn_chat.title
                query.message.edit_text("Successfully connected to *{}*. Use /connection to see current available commands.".format(chat_name), parse_mode=ParseMode.MARKDOWN)
                sql.add_history_conn(user.id, str(conn_chat.id), chat_name)
            else:
        ...
        query.message.edit_text("Connection history has been cleared!")
    elif connect_close:
...
dispatcher.add_handler(CommandHandler("connect", connect_chat, pass_args=True))
dispatcher.add_handler(CommandHandler("connection", connection_chat))
dispatcher.add_handler(CommandHandler("disconnect", disconnect_chat))
dispatcher.add_handler(CommandHandler("allowconnect", allow_connections, pass_args=True))
dispatcher.add_handler(CommandHandler("helpconnect", help_connect_chat, pass_args=True))
dispatcher.add_handler(CallbackQueryHandler(connect_button, pattern=r"connect"))
