...
from TPBOT import DEL_CMDS, SUDO_USERS, WHITELIST_USERS

# from TPBOT.modules import languages
	...
	def delete_rights(update, context, *args, **kwargs):
		if can_delete(update.effective_chat, context.bot.id):
			...
			update.effective_message.reply_text("I can't delete messages here! "
												"Make sure I'm admin and can delete other users' messages.")
...
def user_admin_no_reply(func):
	...
	def is_admin(update, context, *args, **kwargs):
		...
		if user and is_user_admin(update.effective_chat, user.id):
			...
			context.bot.answer_callback_query(update.callback_query.id, "You are not an admin in this group!")
