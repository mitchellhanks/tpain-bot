...
from TPBOT import LOGGER
from TPBOT.modules.users import get_user_id
...
def extract_unt_fedban(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found") and not str(user_id).isdigit():
            message.reply_text("I don't seem to have interacted with this user before - please forward a message from them "
                               "to give me control! (like a voodoo doll, I need a piece of them "
                               "to be able to execute certain commands...)")
            return None, None
