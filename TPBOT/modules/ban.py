...
from TPBOT import dispatcher, BAN_STICKER, LOGGER, spamcheck
from TPBOT.modules.disable import DisableAbleCommandHandler
from TPBOT.modules.helper_funcs.chat_status import bot_admin, user_admin, is_user_ban_protected, can_restrict, \
    is_user_admin, is_user_in_chat
from TPBOT.modules.helper_funcs.extraction import extract_user_and_text
...
def ban(update, context):
    ...
    log = "<b>{}:</b>" \
          ...
          "\n<b>User:</b> {} (<code>{}</code>)".format(html.escape(chat.title),
                                                       ...
                                                       mention_html(member.user.id, member.user.full_name),
                                                       member.user.id)
    if reason:
    ...
    try:
        if conn:
            ...
            send_message(update.effective_message, "Banned from *{}*! 😝".format(chat_name), parse_mode="markdown")
        else:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.exception("ERROR removing user %s from chat %s (%s) caused by %s", user_id, chat.title, chat.id,
                             excp.message)
...
def temp_ban(update, context):
    ...
    if check.status == 'member':
        if conn:
            text = "I can't restrict people in *{}*! Make sure I'm an admin and can appoint a new admins.".format(chat_name)
        else:
            text = "I can't restrict people here! Make sure I'm admin and can appoint new admins."
        send_message(update.effective_message, text, parse_mode="markdown")
        return ""
    ...
    try:
        if conn:
            ...
            # context.bot.send_sticker(currentchat.id, BAN_STICKER)  # banhammer marie sticker
            send_message(update.effective_message, "Banned! User banned for *{}* from *{}*.".format(time_val, chat_name), parse_mode="markdown")
        else:
...
def kick(update, context):
    ...
    if user_id == context.bot.id:
        send_message(update.effective_message, "I'm not going to kick myself, are you crazy? 😠")
        return ""

    if is_user_ban_protected(chat, user_id):
        send_message(update.effective_message, "I can't kick this guy because he's an admin 😒")
        return ""

    if user_id == context.bot.id:
        send_message(update.effective_message, "Yeahhh I wouldn't do that 😝")
        return ""
    ...
    if res:
        if conn:
            # context.bot.send_sticker(currentchat.id, BAN_STICKER)  # banhammer marie sticker
            text = "Kicked from *{}*! 😝".format(chat_name)
            send_message(update.effective_message, text, parse_mode="markdown")
        else:
...
def unban(update, context):
    ...
    if is_user_in_chat(chat, user_id):
        send_message(update.effective_message, "Why are you trying to unban someone who's already in the chat? 😑")
        return ""
    ...
    if conn:
        ...
        send_message(update.effective_message, "Yes, this user can join {}! 😁".format(chat_name))
    else:
        chat.unban_member(user_id)
        send_message(update.effective_message, "Yes, this user can join! 😁")
...
dispatcher.add_handler(CommandHandler(["ban", "sban"], ban, pass_args=True))
dispatcher.add_handler(CommandHandler(["tban", "tempban"], temp_ban, pass_args=True))
dispatcher.add_handler(CommandHandler(["kick", "skick"], kick, pass_args=True))
dispatcher.add_handler(CommandHandler("unban", unban, pass_args=True))
dispatcher.add_handler(DisableAbleCommandHandler("kickme", kickme, filters=Filters.chat_type.groups))
