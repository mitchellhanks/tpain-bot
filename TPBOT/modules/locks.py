...
import TPBOT.modules.sql.locks_sql as sql
from TPBOT import dispatcher, spamcheck, SUDO_USERS, LOGGER
from TPBOT.modules.disable import DisableAbleCommandHandler
from TPBOT.modules.helper_funcs.chat_status import (
    can_delete,
    is_user_admin,
    user_not_admin,
    user_admin,
...
from TPBOT.modules.sql import users_sql
...
__help__ = """
View or set global restrictions for the group. (e.g. forwarding, stickers, bots, URLs, etc.)
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = f"Permission *{ltype}* was locked for all non-admins at *{chat_name}*!"
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(update.effective_message, "You can do this command in groups, not PM")
                        return ""
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = f"Permission *{ltype}* was locked for all non-admins!"
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = f"Unlocked {ltype} for everyone at *{chat_name}*!"
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(update.effective_message, "You can do this command in groups, not PM")
                        return ""
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = f"Permission *{ltype}* was unlocked for all non-admins at *{chat_name}*!"
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(update.effective_message, "You can do this command in groups, not PM")
                        return ""
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = f"Permission *{ltype}* was unlocked for all non-admins!"
                ...
                send_message(update.effective_message, "What are you trying to unlock...? Try /locktypes for the lockable lock list")
                    ...
                    if "ARABIC" in check:
                        ...
                        if getconf:
                            warn(
                                ...
                                chat,
                                "Sending 'RTL Text' is currently locked",
                                message,
                                update.effective_user,
                                conn=False,
                            )
                ...
                if message.reply_markup and message.reply_markup.inline_keyboard:
                    ...
                    if getconf:
                        warn(
                            update.effective_user,
                            chat,
                            "Send 'Button Message' which currently locked",
                            message,
                            update.effective_user,
                            conn=False,
                        )
                ...
                if message and message.via_bot:
                    ...
                    if getconf:
                        warn(
                            update.effective_user,
                            chat,
                            f"Send '{lockable}' which currently locked",
                            message,
                            update.effective_user,
                            conn=False,
                        )
                    ...
                    if new_mem.is_bot:
                        ...
                        if getconf:
                            warn(
                                update.effective_user,
                                chat,
                                "Invite 'Bot' which currently locked",
                                message,
                                update.effective_user,
                                conn=False,
                            )
...
def build_lock_message(chat_id):
    ...
    if locklist:
        ...
        for x in locklist:
            res += "\n - {}".format(x)
    res += "\n\n*These are the permissions in this chat:*"
    for x in permslist:
        res += "\n - {}".format(x)
    return res
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace("this chat", f"*{chat_name}*")
...
def lock_warns(update, context):
    ...
    if args:
        if args[0] == "on" or args[0] == "yes":
            ...
            try:
                send_message(update.effective_message, "I *will warn* user if send any message/media which currently locked")
            except BadRequest:
                send_message(update.effective_message, "I *will warn* user if send any message/media which currently locked", quote=False)
        elif args[0] == "off" or args[0] == "no":
            sql.set_lockconf(chat_id, False)
            try:
                send_message(update.effective_message, "I *will not warn* user if send any message/media which currently locked")
            except BadRequest:
                send_message(update.effective_message, "I *will not warn* user if send any message/media which currently locked", quote=False)
        else:
            try:
                send_message(update.effective_message, "I only understand 'on/yes' or 'off/no'!")
            except BadRequest:
                send_message(update.effective_message, "I only understand 'on/yes' or 'off/no'!", quote=False)
    else:
        getconf = sql.get_lockconf(chat_id)
        if getconf:
            try:
                send_message(update.effective_message, "Currently I *will warn* user if send any message/media which currently locked")
            except BadRequest:
                send_message(update.effective_message, "Currently I *will warn* user if send any message/media which currently locked", quote=False)
        else:
            try:
                send_message(update.effective_message, "Currently I *will not warn* user if send any message/media which currently locked")
            except BadRequest:
                send_message(update.effective_message, "Currently I *will not warn* user if send any message/media which currently locked", quote=False)
...
dispatcher.add_handler(DisableAbleCommandHandler("locktypes", locktypes))
dispatcher.add_handler(CommandHandler("locks", list_locks))
dispatcher.add_handler(CommandHandler("lockwarns", lock_warns, pass_args=True))
