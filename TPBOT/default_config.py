if not __name__.endswith("default_config"):
    import sys
    print("ERROR: Improper config.  Extend the default_config.py to a config file, don't just rename and change "
          "values here. Check the README for more info\nBot quitting.", file=sys.stderr)
    quit(1)
...
class Config(object):
    ...
    # CAUTION!!  If any IDs are listed here, it will make the bot "private" where only the groups listed may use it.
    # This will override the GROUP_BLACKLIST option
    GROUP_WHITELIST = []  # List of id's (not groupnames) for groups which are permitted to use the bot.
